...
namespace TsBlog.Core
{
    public static class ValidCodeUtils //公共静态
    {
        ...
        public static byte[] CreateImage(string strRandom)
        {
            ...
            //在最外边绘制边框
            g.DrawRectangle(new Pen(Color.FromArgb(204,204,204)), 0, 0, newBitmap.Width, newBitmap.Height);
            g.DrawRectangle(new Pen(Color.FromArgb(204, 204, 204)), -1, -1, newBitmap.Width, newBitmap.Height);
            //将图转保存到内存流中
